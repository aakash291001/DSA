...
class tree
{
    ...
    void pairsum(node1 *p,int sum)
    {
    	...
    	else
    	{
    		...
    		{
    			bool l,r;
    			l = Bsearch(sum-p->data,root);
    			r = Bsearch(sum-p->data,root);
    			if(r||l)
    			{
    				cout<<p->data<<" "<<sum-p->data;
				}
    			pairsum(p->lchild,sum);
    			pairsum(p->rchild,sum);
			}
			else
			{
				pairsum(p->lchild,sum);
